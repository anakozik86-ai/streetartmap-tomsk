...
interface FormErrors { id?: string; name?: string; color?: string; }
...
export function CollectionsEditor(): JSX.Element {
  ...
  function openNew(): void {
    setEditing({ id: '', type: 'series', name: '', color: '#4DB8FF', description: '',
                 year_start: '', year_end: '', organizer_or_author: '', status: 'active' });
    setIsNew(true);
    setErrors({});
  }
  ...
  function openEdit(c: Collection): void {
    setEditing({
      id: c.id, type: c.type, name: c.name, color: c.color,
  ...
  async function submit(e: Event): Promise<void> {
    ...
    const col: Collection = {
      ...
      year_start:          editing.year_start ? Number(editing.year_start) : undefined,
      year_end:            editing.year_end   ? Number(editing.year_end)   : undefined,
      organizer_or_author: editing.organizer_or_author.trim() || undefined,
    ...
    try { await saveCollection(col); close(); } catch { /* saveState отображает */ }
  }

  async function doArchive(id: string): Promise<void> {
    const login = pat.value.split(':')[0] ?? 'admin';
    setArchiving(id);
    try { await archiveCollection(id, login); } finally { setArchiving(null); }
  }
  ...
  return (
    <div class="cat-editor">
      ...
      <table class="admin-table">
        <thead>
          <tr><th>ID</th><th>Тип</th><th>Название</th><th>Цвет</th><th></th></tr>
        </thead>
        <tbody>
          {active.map((c) => (
            <tr key={c.id}>
        ...
        <Modal title={isNew ? 'Новая коллекция' : 'Редактировать коллекцию'} onClose={close}>
          <form onSubmit={submit}>
            ...
            <Field label="Тип">
              <Select
                value={editing.type}
                onChange={(e) => setEditing({ ...editing, type: (e.target as HTMLSelectElement).value as 'festival' | 'series' })}
              >
            ...
            <Field label="Цвет обводки маркера" required error={errors.color}>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <input
                  ...
                  style={{ width: '2.5rem', height: '2rem', padding: '0', border: 'none', cursor: 'pointer', borderRadius: 'var(--radius-sm)', flexShrink: '0' }}
                />
                <Input
                  value={editing.color}
                  onInput={(e) => setEditing({ ...editing, color: (e.target as HTMLInputElement).value })}
            ...
            <Field label="Организатор / автор">
              <Input
                ...
                onInput={(e) => setEditing({ ...editing, organizer_or_author: (e.target as HTMLInputElement).value })}
              />
            </Field>
            <Field label="Статус">
              <Select
