...
export interface Category extends Auditable {
  ...
  description?: string; order: number; status: EntityStatus;
}

export type CollectionType = 'festival' | 'series';
export interface Collection extends Auditable {
  id: string; type: CollectionType; name: string; color: string;
  description?: string; year_start?: number; year_end?: number;
  organizer_or_author?: string; cover_image?: string;
  external_links?: ExternalLink[]; status: EntityStatus;
}
...
export interface Author extends Auditable {
  ...
  origin?: string; external_links?: ExternalLink[]; status: EntityStatus;
}
...
export interface PointPhoto {
  filename: string; caption?: string; credit?: string;
...
export interface Point extends Auditable {
  ...
  address_hint?: string; accessibility: PointAccessibility;
  category_id: string; collection_ids: string[]; tags: string[];
  title: string; description: string;
  author_id?: string; year_created?: number;
  dimensions?: string; materials: string[];
  state: PointState; state_checked_at?: string;
  photos: PointPhoto[]; featured: boolean;
}

export interface RouteGeometry { type: 'LineString'; coordinates: [number, number][]; }
export interface Route extends Auditable {
  id: string; status: ContentStatus; name: string; description?: string;
  point_ids: string[];
  /**
  ...
  geometry: RouteGeometry | null; geometry_hash: string;
  total_distance_m?: number; total_duration_s?: number; cover_image?: string;
}

export interface SiteConfig {
  schema_version: number;
  city: { name: string; name_en?: string; center: {lat:number;lng:number}; default_zoom: number;
